'use client'
import { ArrowRight } from 'lucide-react'
import Link from 'next/link'

interface TitleProps {
    title: string
    description: string
    visibleButton?: boolean
    href?: string
}

const Title = ({ title, description, visibleButton = true, href = '' }: TitleProps) => {

    return (
        <div className='flex flex-col items-center'>
            <h2 className='text-2xl font-bold text-slate-800'>{title}</h2>
            <Link href={href} className='flex items-center gap-5 text-sm text-slate-600 mt-2'>
                <p className='max-w-lg text-center'>{description}</p>
                {visibleButton && <button className='text-indigo-600 flex items-center gap-1 font-medium hover:text-indigo-700 transition-colors'>View more <ArrowRight size={14} /></button>}
            </Link>
        </div>
    )
}

export default Title