...
import type { Store } from "@/assets/assets"
...
const StoreInfo = ({ store }: StoreInfoProps) => {
    const logoSrc = typeof store.logo === "string" ? store.logo : store.logo?.src ?? ""
    const userImageSrc = typeof store.user?.image === "string" ? store.user.image : store.user?.image?.src ?? ""
        ...
        <div className="flex-1 space-y-2 text-sm">
            ...
            {store.user ? (
                <div className="flex items-center gap-2 text-sm ">
                    <Image width={36} height={36} src={userImageSrc} alt={store.user.name} className="w-9 h-9 rounded-full" />
                    <div>
